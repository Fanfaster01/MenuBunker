...
export default function GroupEditModal({ group, onClose, onSave }) {
  ...
  async function handleSubmit(e) {
    ...
    const result = await onSave(group.codigo, group.departamento_codigo, patch);
    setSaving(false);
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-linear-to-r from-red-50 to-white">
          <div>
            <h2 className="text-lg font-bold text-[#6B5A45]">Editar grupo</h2>
            <p className="text-xs text-gray-500 mt-0.5">
              ERP: <span className="font-mono">{group.raw_name}</span> · #{group.codigo} · {group.department_name} · {group.product_count} productos
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="px-6 py-5 space-y-5">
            <Field label="Nombre visible" help={`Si lo dejas vacío, se usa el del ERP: "${group.raw_name}"`}>
              <input
                type="text"
                value={values.display_name}
                onChange={(e) => setField('display_name', e.target.value)}
                placeholder={group.raw_name}
                maxLength={100}
                className="input"
              />
            </Field>

            <Field label="Slug (URL)" help={`URL final: /la-victoriana/${group.department_slug || '…'}/{slug}`}>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500 font-mono truncate">
                  /la-victoriana/{group.department_slug || '…'}/
                </span>
                <input
                  type="text"
                  value={values.slug}
            ...
            <Field label="Descripción (subtítulo)" help="Texto corto que aparece debajo del nombre.">
              <input
                ...
                placeholder="Ej: Pollo fresco nacional"
                maxLength={200}
                className="input"
              />
            </Field>
